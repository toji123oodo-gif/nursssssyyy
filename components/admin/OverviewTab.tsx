        ...
        {stats.map((s, i) => (
          <div key={i} className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-2">
               <div className="text-gray-500 bg-gray-50 p-2 rounded-md">
                 <s.icon size={20} />
               </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg border border-gray-200 shadow-sm p-6">
           <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-gray-900">تحليل المشاهدات</h3>
              <select className="bg-gray-50 text-sm border border-gray-200 rounded-md px-3 py-1.5 outline-none focus:border-brand-blue">
           ...
           <div className="flex items-end justify-between h-48 gap-2 px-2">
              {[40, 70, 55, 90, 65, 85, 100].map((h, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                   <div className="w-full bg-blue-50 rounded-t-sm relative overflow-hidden transition-all hover:bg-blue-100" style={{ height: `${h}%` }}></div>
